...
import getToastsFromPyFlashMessages from '../../../features/messageToasts/utils/getToastsFromPyFlashMessages';
...
export default function getInitialState({
  ...
  if (
    ...
    if (sqlSupport.queryEditors.length === 0) {
      ...
      sqlSupport.queryEditors.forEach(qe =>
        queryEditors.push({
          ...qe,
          inLocalStorage: true,
          loaded: true,
      ...
      sqlSupport.tables.forEach(table =>
        tables.push({ ...table, inLocalStorage: true }),
      );
      Object.values(sqlSupport.queries).forEach(query => {
        queries[query.id] = { ...query, inLocalStorage: true };
      });
      tabHistory.push(...sqlSupport.tabHistory);
    }
  }
