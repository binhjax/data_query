...
class TabbedSqlEditors extends React.PureComponent {
  constructor(props) {
    super(props);
    const sqlSupportUrl = '/tekodatewarehouse/sqlsupport';
    this.state = {
      sqlSupportUrl,
      queriesArray: [],
      dataPreviewQueries: [],
  ...
  componentDidMount() {
    ...
    if (
      ...
      if (query.new) {
        window.history.replaceState({}, document.title, this.state.sqlSupportUrl);
      }
    } else {
...
function mapStateToProps({ sqlSupport, common, requestedQuery }) {
  return {
    databases: sqlSupport.databases,
    queryEditors: sqlSupport.queryEditors,
    queries: sqlSupport.queries,
    tabHistory: sqlSupport.tabHistory,
    tables: sqlSupport.tables,
    defaultDbId: sqlSupport.defaultDbId,
