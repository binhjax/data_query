...
const { supersetPort = 8080, superset: supersetUrl = null } = parsedArgs;
...
function toDevHTML(originalHtml) {
  console.log("binhnt: toDevHTML start")
    ...
    html = html.replace(
      ...
      (match, assetType, bundleName) => {
        if (bundleName in manifest.entrypoints) {
          console.log("binhnt: start embeded bundle: ", bundleName)
          return `<!-- DEV bundle: ${bundleName} ${assetType} START -->\n  ${(
            manifest.entrypoints[bundleName][assetType] || []
          )
            .filter(chunkFilePath => {
              console.log("binhnt: start embeded bundle: ", bundleName, " -> filter ")
...
//binhnt: copy header from backend to frontend 
function copyHeaders(originalResponse, response) {
  console.log("binhnt: copyHeaders start")
...
function processHTML(proxyResponse, response) {
  console.log("binhnt: processHTML start")
...
module.exports = newManifest => {
  ...
  return {
    ...
    onProxyRes(proxyResponse, request, response) {
      console.log("binhnt: onProxyRes start")
      try {
        copyHeaders(proxyResponse, response);
        if (isHTML(response)) {
          console.log("binhnt: isHTML => call processHTML")
          processHTML(proxyResponse, response);
        } else {
          console.log("binhnt: not isHTML => call proxyResponse.pipe")
